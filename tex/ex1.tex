...
\begin{abstract}
This \LaTeX\ template can be used to prepare a research article or a short article for submission to OSA's new journal, \emph{Optica}. Select \texttt{$\setminus$setboolean\{shortarticle\}\{false\}} for a research article or \texttt{$\setminus$setboolean\{shortarticle\}\{true\}} for a letter. Select \texttt{$\setminus$setboolean\{minireview\}\{true\}} to output a header identifying the paper as a Mini-Review. The template can also be used to create a supplementary document (see the details provided below). Note that this template can be run from your own \TeX\ system or within the cloud-based \href{https://www.overleaf.com}{Overleaf} system.
\end{abstract}
