...
\hline
 \textbf{ID} & \textbf{Operation} & \textbf{Time Complexity} \\ \hline
 1 & Creating $C$ random chromosomes & $\mathcal{O}(C^2N)$ \\ \hline
 2 & Sorting the array of $C$ chromosomes & $\mathcal{O}(C\log{C})$ \\ \hline
 3 & Creating $\frac{3C}{4}$ child chromosomes & $\mathcal{O}(CN)$ \\ \hline
 4 & Multithreading & $\mathcal{O}(P)$ \\ \hline
\end{tabular}
\caption{Complexity Analysis of the Algorithm}
\end{table}

We can see that operation 1 has the highest time complexity. Thus,  $\mathcal{O}(C^2N)$ is the complexity of our algorithm.
